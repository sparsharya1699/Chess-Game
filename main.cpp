...
int main()
{
    ...
    SELECT_PIECE:
        ...
        coord_x = chx - '0';
        coord_y = chy - '0';
        if (gameSquares[coord_x][coord_y].kill_him != whose_turn)
        {
            goto WRONG_PIECE;
        }
        else
...
void CPiece::highlightPiece(int cx, int cy, int thisPiece)
{
    ...
    if (thisPiece == PAWN)
    {
        ...
        { // If it is Player's PAWN
            if (cy == 2)
            { // If this is first action of pawn
                printPiece(cx, cy, 14, PAWN);
                for (int i = cy, j = 0; j < 2; i++, j++)
                { // j<2 for 2 steps
                    if (gameSquares[cx][i + 1].is_empty == 0 && gameSquares[cx][i + 1].kill_him == 0)
                    {
                        break;
                    }
                    else if (gameSquares[cx][i + 1].is_empty == 0 && gameSquares[cx][i + 1].kill_him == 1)
                    {
                        break;
                    }
                    else
        ...
        { // If it is Computer's PAWN
            if (cy == 7)
            { // If this is first action of pawn
                printPiece(cx, cy, 14, PAWN);
                for (int i = cy - 1, j = 0; j < 2; i--, j++)
                { // j<2 for 2 steps
                    if (gameSquares[cx][i].is_empty == 0 && gameSquares[cx][i].kill_him == 0)
                    {
                        break;
                    }
                    else if (gameSquares[cx][i].is_empty == 0 && gameSquares[cx][i].kill_him == 1)
                    {
                        break;
                    }
                    else
